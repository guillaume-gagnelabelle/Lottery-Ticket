...
'''
 ...
 It graphs the performance of the saved models.
'''
...
for log in logs:
    ...
    for seed in seeds:
        if args.perf_type == "lottery_ticket":
            dicts.append(torch.load(f"{os.getcwd()}/saves/{args.arch_type}/{args.dataset}/final_training_v2/{log}_seed{seed}_co2False_{args.dataset}.pt", map_location=torch.device('cpu')))
        elif args.perf_type == "hyperparameter":
